...
from ISTKHAR_MUSIC.mongo.afkdb import LOGGERS as OWNERS
from ISTKHAR_MUSIC.utils.database import add_served_chat, get_assistant


@app.on_message(filters.command("repo"))
async def help(client: Client, message: Message):
    await message.reply_photo(
        photo=f"https://files.catbox.moe/q9shgz.jpg",
        caption=f"""🍁𝐂𝐋𝐈𝐂𝐊🥰𝐁𝐄𝐋𝐎𝐖💝𝐁𝐔𝐓𝐓𝐎𝐍✨𝐓𝐎🙊𝐆𝐄𝐓🌱𝐑𝐄𝐏𝐎🍁""",
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "ƨσʋяcɛ", url=f"https://github.com/TEAM-ISTKHAR/ISTKHAR_MUSIC"
                    )
                ]
            ]
        ),
...
async def add_allbot(client, message):
    ...
    if len(command_parts) != 2:
        await message.reply(
            "**⚠️ ɪɴᴠᴀʟɪᴅ ᴄᴏᴍᴍᴀɴᴅ ғᴏʀᴍᴀᴛ. ᴘʟᴇᴀsᴇ ᴜsᴇ ʟɪᴋᴇ » `/gadd @Sanatani_music_xd_bot`**"
        )
        return
